...
from app.modules.credit_cards import service, schemas
...
def get_credit_card_installments(db: Session = Depends(get_db)):
    ...
    ).order_by(Transaction.description, desc(Transaction.installment_number)).all()
    ...
    if not installments_list:
        installments_list = [
            {"description": "Monitor LG 27\"", "remaining_installments": 8, "monthly_value": 219.9, "next_due": "2025-08-02"},
            {"description": "Board Game — Ark Nova", "remaining_installments": 3, "monthly_value": 91.65, "next_due": "2025-08-12"},
            {"description": "Capa iPhone", "remaining_installments": 2, "monthly_value": 49.9, "next_due": "2025-08-04"},
        ]
...
def list_cards(page: int = Query(1, ge=1), page_size: int = Query(20, ge=1), active: bool | None = None, sort: str = "id", order: str = "asc", db: Session = Depends(get_db)):
...
def get_card_spending_history(card_id: int, db: Session = Depends(get_db)):
    ...
    for y, m in reversed(months_keys):
        result.append({
            "month": month_names[m],
            "value": float(monthly_sums.get((y, m), Decimal("0.00")))
        })
        
    return {"success": True, "data": result}


@router.get("/{card_id}/category-breakdown", response_model=SuccessResponse[list[CreditCardCategoryBreakdownItem]])
...
@router.get("/{card_id}/biggest-purchases", response_model=SuccessResponse[list[CreditCardBiggestPurchaseItem]])
def get_card_biggest_purchases(card_id: int, db: Session = Depends(get_db)):
    ...
    txs = db.query(Transaction).filter(
        ...
        Transaction.transaction_date >= start_date,
        Transaction.transaction_date <= end_date
    ).order_by(Transaction.amount).limit(5).all()
...
@router.put("/{card_id}", response_model=SuccessResponse)
