...
def get_account_balance_history(account_id: int, db: Session = Depends(get_db)):
    ...
    txs = (
        ...
        .filter(Transaction.account_id == account_id, Transaction.status == "cleared")
    ...
    for y, m in reversed(months_keys):
        result.append(
            {"month": month_names[m], "value": float(balances.get((y, m), Decimal("0.00")))}
        )
...
@router.get(
    "/{account_id}/transactions", response_model=SuccessResponse[PageResponse[TransactionDTO]]
)
def get_account_transactions(
    ...
    items, total = svc.list(
        page=page, page_size=page_size, filters=filters, sort="transaction_date", order="desc"
    )
    dto_items = [TransactionDTO.model_validate(item) for item in items]
    return {
        "success": True,
        "data": {"items": dto_items, "page": page, "page_size": page_size, "total": total},
    }
